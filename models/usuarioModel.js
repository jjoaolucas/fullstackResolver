...
class UsuarioModel {
    ...
    constructor(usuarioIdCript, usuarioNome, 
    ...
    async buscarUsuario(id){
        ...
        if(rows.length > 0){
            return new UsuarioModel(criptografa.criptografa(rows[0]["usu_id"]), rows[0]["usu_nome"], rows[0]["usu_email"], rows[0]["usu_senha"], rows[0]["usu_ativo"],
            rows[0]["per_id"]);
        }
        return null;
    }
    ...
    async autenticarUsuario(email, senha){
        ...
        let rows = await conexao.ExecutaComando(sql, valores);
        if(rows.length > 0){
            return new UsuarioModel(cripto.criptografa(rows[0]["usu_id"]), rows[0]["usu_nome"], rows[0]["usu_email"], rows[0]["usu_senha"], rows[0]["usu_ativo"],
            rows[0]["per_id"]);
        }
